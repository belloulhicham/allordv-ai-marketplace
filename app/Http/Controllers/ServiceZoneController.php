...
class ServiceZoneController extends Controller
{
    ...
    public function index_data(DataTables $datatable, Request $request)
    {
        try {
            ...
            return $datatable->eloquent($query)
                ...
                ->addColumn('ai_sector', function ($row) {
                    return $row->ai_sector ? '<span class="badge bg-primary">' . e($row->ai_sector) . '</span>' : '<span class="text-muted">Non défini</span>';
                })
                ->addColumn('ai_model', function ($row) {
                    return $row->ai_model ? '<span class="badge bg-success">' . e($row->ai_model) . '</span>' : '<span class="text-muted">Non défini</span>';
                })
                ...
                ->rawColumns(['check', 'name', 'status', 'action', 'providers', 'service_count', 'ai_sector', 'ai_model'])
                ->toJson();
        } catch (\Exception $e) {
    ...
    {
        try {
            // Pour les agents IA, on peut utiliser d'autres critères que les coordonnées géographiques
            // Pour l'instant, on garde la logique existante pour la compatibilité
    ...
    public function create(Request $request)
    {
        ...
        if ($servicezone) {
            // Gérer ai_parameters pour l'édition
            if (is_string($servicezone->ai_parameters)) {
                $servicezone->ai_parameters = json_decode($servicezone->ai_parameters, true);
            }
            
            // S'assurer que les nouvelles colonnes sont bien chargées
            $servicezone->ai_sector = $servicezone->ai_sector ?? null;
            $servicezone->ai_personality = $servicezone->ai_personality ?? null;
            $servicezone->ai_specializations = $servicezone->ai_specializations ?? [];
            $servicezone->ai_model = $servicezone->ai_model ?? 'gpt-4o';
            $servicezone->api_key = $servicezone->api_key ?? null;
            ...
            $servicezone->ai_parameters = [];
            $servicezone->ai_sector = null;
            $servicezone->ai_personality = null;
            $servicezone->ai_specializations = [];
            $servicezone->ai_model = 'gpt-4o';
            $servicezone->api_key = null;
        }

        // Debug des paramètres IA
        \Log::info('Service Zone AI Parameters:', [
            'zone_id' => $id,
            'ai_parameters' => $servicezone->ai_parameters,
            'ai_sector' => $servicezone->ai_sector,
            'ai_personality' => $servicezone->ai_personality,
            'ai_specializations' => $servicezone->ai_specializations,
            'ai_model' => $servicezone->ai_model,
            'api_key' => $servicezone->api_key,
        ]);
    ...
    public function store(Request $request)
    {
        ...
        // Nouvelle validation pour les champs IA
        $request->validate([
            'name' => 'required|string|unique:service_zones,name,' . ($request->id ?? 'NULL') . ',id',
            'ai_sector' => 'required|string|max:255',
            'ai_personality' => 'nullable|string|max:255',
            'ai_specializations' => 'nullable|array',
            'ai_model' => 'required|string|max:255',
            'api_key' => 'nullable|string',
            'ai_parameters' => 'nullable|json',
            'status' => 'nullable|boolean',
        ], [
            'ai_parameters.json' => trans('messages.ai_parameters_json_invalid'),
        ]);

        try {
            $data = $request->all();

            // Gérer ai_specializations (conversion tableau -> JSON)
            if (isset($data['ai_specializations']) && is_array($data['ai_specializations'])) {
                $data['ai_specializations'] = json_encode($data['ai_specializations']);
            } else {
                $data['ai_specializations'] = null;
            }

            // Gérer ai_parameters
            if (isset($data['ai_parameters']) && !empty($data['ai_parameters'])) {
                try {
                    json_decode($data['ai_parameters']);
                    if (json_last_error() !== JSON_ERROR_NONE) {
                        return redirect()->back()->withErrors(['ai_parameters' => trans('messages.ai_parameters_json_invalid')])->withInput();
                    }
                } catch (\Exception $e) {
                    return redirect()->back()->withErrors(['ai_parameters' => trans('messages.ai_parameters_json_invalid')])->withInput();
                }
            } else {
                $data['ai_parameters'] = null;
            }

            // Nettoyer les champs non nécessaires
            unset($data['status_checkbox']);
            unset($data['coordinates']); // Plus utilisé
            ...
            $result = ServiceZone::updateOrCreate(
                ...
                [
                    ...
                    'ai_sector' => $data['ai_sector'],
                    'ai_personality' => $data['ai_personality'],
                    'ai_specializations' => $data['ai_specializations'],
            ...
            Log::error('Error storing service zone: ' . $e->getMessage(), ['exception' => $e]);
            ...
            return redirect()->back()->withErrors(['error' => 'An error occurred while saving the service zone: ' . $e->getMessage()]);
        }
    }

    public function edit($id)
    {
        // Méthode vide maintenant que create() gère l'édition
    }

    public function update(Request $request, $id)
    {
        // Méthode vide maintenant que store() gère la mise à jour via updateOrCreate
    }
