  ...
  <div class="iq-image position-relative">
   ...
   <ul class="list-inline p-0 mt-1 mb-0 price-content">
    <li class="text-primary fw-500 d-inline-block position-relative font-size-18">
        <!-- Si c'est un agent IA -->
        <span v-if="is_ai_agent">
            {{ price_per_session_mad }} MAD/session
        </span>
        <!-- Sinon prix normal -->
        <span v-else-if="price>0">
            {{ formatCurrencyVue(price) }} 
            <span v-if="discount && discount > 0"> ({{ discount }}% off)</span>
        </span>
        <span v-else>Free</span>
    </li>
    <!-- Masquer durée pour agents IA -->
    <li v-if="!is_ai_agent && duration && duration !== '00:00'" class="d-inline-block fw-500 position-relative service-price">
...
const props = defineProps({
 ...
 is_ai_agent: {type: Boolean, default: false},
    ai_speciality: {type: String, default: ''},
    price_per_session_mad: {type: String, default: '0'},
...
<!-- NOUVEAU: Badge Agent IA -->
<span v-if="is_ai_agent" class="badge bg-primary position-absolute top-0 start-0 m-2">
   🤖 Agent IA
</span>
